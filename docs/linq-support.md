...
| `ExecuteUpdateAsync` | Constrained | Simple filtered entity queries only. Grouped, ordered, joined, distinct, paged, and aggregated queries are rejected. Two assignment forms supported: (1) literal / captured-local value — `SetProperty(x => x.Foo, 42)`; (2) server-side computed expression over the row — `SetProperty(x => x.Counter, x => x.Counter + 1)`. The computed form supports member access on the row, primitive constants, arithmetic operators (`+`/`-`/`*`/`/`/`%`), and primitive Convert. Other shapes throw with an actionable rewrite hint. |
| `ExecuteDeleteAsync` | Constrained | Filtered entity queries, including correlated `EXISTS`/`NOT EXISTS` subqueries via `Where(p => !ctx.Query<Other>().Any(o => o.K == p.K))`. Grouped, ordered, joined, distinct, paged, and aggregated queries are still rejected. |
