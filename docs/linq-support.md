...
| Group joins | Constrained | Supported for simple key joins and bounded by `DbContextOptions.MaxGroupJoinSize`. Async streaming with group joins is not supported. The `GroupJoin` + `SelectMany` + `DefaultIfEmpty` (query-syntax left join) composition is supported including projections that contain idiomatic null-guards on the inner entity (`c == null ? null : c.Prop` / `c != null ? c.Prop : null`) — the translator strips the redundant guard since LEFT JOIN already NULLs unmatched rows. Non-null fallback variants (`c == null ? "None" : c.Prop`) are not yet supported. |
