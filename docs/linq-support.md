...
| Scalar operators after `Append`/`Prepend`/`Chunk`/`Zip`/`DefaultIfEmpty(value)` | Supported | The source query runs server-side; the reshaped sequence is evaluated in memory with exact LINQ-to-Objects semantics. `Count`, `LongCount`, `Sum`, `Min`, `Max`, `Average`, `Any`, `All`, and `Contains` (including selector/predicate overloads) reduce the reshaped rows client-side, and `Where` after a reshape filters the reshaped sequence — the appended/prepended/default element passes through the same predicate as the rows. |
