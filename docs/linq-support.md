...
| `Zip` | Constrained | Supported when the second sequence is a constant or captured local collection, with or without a result selector; pairing truncates to the shorter side per LINQ semantics. Zipping two database queries throws `NormUnsupportedFeatureException` (positional pairing of two result sets has no provider-mobile SQL translation). |
