...
| Inner joins | Supported | `Join` translates to provider SQL and is included in benchmark parity gates. Member-initialized DTO projections and computed projection members are covered by strict-mode and live-provider parity tests. |
...
| Raw SQL composition | Provider-bound compatibility API | `FromSqlRawAsync<T>` and `FromSqlInterpolatedAsync<T>` execute read-only `SELECT`/CTE statements through the provider-aware raw query gate. Raw SQL does not add tenant predicates automatically and is not provider-mobile generated surface. `UseStrictProviderMobility()` rejects raw SQL APIs before execution. |
