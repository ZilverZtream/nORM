...
| Scalar operators after `Append`/`Prepend`/`Chunk`/`Zip`/`DefaultIfEmpty(value)` | Constrained | Row-materializing terminals (`First`, `Single`, `ElementAt`, `ToListAsync`, …) see the reshaped sequence. Server-scalar operators (`Count`, `LongCount`, `Any`, `All`, `Contains`, `Sum`, `Min`, `Max`, `Average`) throw `NormUnsupportedFeatureException` instead of evaluating against pre-reshape server rows — materialize first and aggregate in memory. |
