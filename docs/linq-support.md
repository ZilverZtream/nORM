...
| `GroupBy` | Constrained | Single-column AND composite anonymous-type keys (`p => new { p.A, p.B }`) supported. Within that: `g.Key`, `g.Key.A` member access on composite keys, individual aggregates, multi-aggregate anonymous-type projections, and `Where(g => aggregate ...)` (HAVING) all execute server-side. Arbitrary client `IGrouping` composition is not a v1 guarantee. |
| Inner joins | Supported | `Join` translates to provider SQL and is included in benchmark parity gates. |
| Group joins | Constrained | Supported for simple key joins and bounded by `DbContextOptions.MaxGroupJoinSize`. Async streaming with group joins is not supported. The `GroupJoin` + `SelectMany` + `DefaultIfEmpty` (query-syntax left join) composition is fully supported, including projections that contain idiomatic null-guards (`c == null ? null : c.Prop` / `c != null ? c.Prop : null`) — the translator strips the redundant guard since LEFT JOIN already NULLs unmatched rows — and non-null fallback variants (`c == null ? "None" : c.Prop` / `c != null ? c.Prop : "None"`) which are lowered to `COALESCE(c.Prop, 'None')`. |
| `SelectMany` | Constrained | Supported shapes: cross-product (`from a in A from b in B select new { a.X, b.Y }` → `CROSS JOIN`), inner-join through a navigation (with optional `Where` filter), navigation `+ DefaultIfEmpty()` (→ `LEFT JOIN`), and the query-syntax left join `from p in P join c in C on p.K equals c.K into grp from c in grp.DefaultIfEmpty() select projection(p, c)` (→ `LEFT JOIN` with the GroupJoin's keys, skipping MaterializeGroupJoin). Complex correlated collection expansion is not a full v1 guarantee. |
...
| `ExecuteUpdateAsync` | Constrained | Filtered and join-sourced entity queries. Grouped, ordered, distinct, paged, and aggregated queries are rejected. Join sources are translated to `UPDATE t SET ... WHERE pk IN (SELECT T0.pk FROM t T0 JOIN ...)`. Two assignment forms: (1) literal / captured-local value — `SetProperty(x => x.Foo, 42)`; (2) server-side computed expression over the row — `SetProperty(x => x.Counter, x => x.Counter + 1)`. The computed form supports member access on the row, primitive constants, arithmetic operators (`+`/`-`/`*`/`/`/`%`), and primitive Convert. Composite-key entities with a join source are not supported (use correlated `Any()` instead). |
| `ExecuteDeleteAsync` | Constrained | Filtered and join-sourced entity queries, including correlated `EXISTS`/`NOT EXISTS` subqueries via `Where(p => !ctx.Query<Other>().Any(o => o.K == p.K))`. Join sources are translated to `DELETE FROM t WHERE pk IN (SELECT T0.pk FROM t T0 JOIN ...)`. Grouped, ordered, distinct, paged, and aggregated queries are rejected. Composite-key entities with a join source are not supported. |
...
| `DateTimeOffset` col `==` / `!=` DateTime literal | Constrained | Lowered to UTC-instant equality via per-provider epoch-seconds hooks (SQLite `strftime('%s')`, SqlServer `DATEDIFF_BIG`, Postgres `EXTRACT(EPOCH)`, MySQL `UNIX_TIMESTAMP`). Second-resolution: sub-second fidelity is post-v1. `.Kind == Utc` literals get offset 0; Unspecified / Local literals use the snapshot local offset. |
| `DateTimeOffset` - `DateTimeOffset` → `TimeSpan` (cross-column) | Supported | Integer UTC-epoch-seconds difference. Independent of either column's stored offset. Materialiser reads as REAL seconds via `TimeSpan.FromSeconds`. |
...
| `Enum.TryParse<T>(stringCol, out T)` as WHERE predicate | Constrained | 2-arg generic form only. Lowers to `(col IN ('Name1', 'Name2', ...))` — the out variable is a syntax requirement (expression trees disallow `out _`) but is unused at the SQL level. The richer "TryParse + use out-var in same predicate" shape isn't pattern-matched; use `col == nameof(X)` instead. 3-arg `ignoreCase` form unsupported. |
