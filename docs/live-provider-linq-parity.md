...
| `TakeWhile` / `SkipWhile` ordered subset | ✅ | ✅ | ✅ | ✅ | ✅ | — | Requires explicit `OrderBy`/`ThenBy`; one-argument and index-aware predicate overloads. Lowers to a windowed cumulative break flag and composes with a directly preceding `Take`/`Skip` window via a derived-table wrap; unordered forms fail closed. | `LinqTakeSkipWhileProviderMobileTests`, `LiveProviderTakeSkipWhileParityTests`, `TakeSkipWhileAfterWindowTests` |
...
| `Include` / `ThenInclude` with `AsSplitQuery` | ✅ | ✅ | ✅ | ✅ | ✅ | — | Include eager-loads collection and reference navigations (including nested ThenInclude paths) whenever it is present; `AsSplitQuery` is accepted for compatibility and adds no further effect. | `LiveProviderIncludeParityTests`, `LinqMultiLevelIncludeTests`, `IncludeContractTests` |
...
| `ExecuteUpdateAsync` / `ExecuteDeleteAsync` filtered and join-sourced, including composite PK | ✅ | ✅ | ✅ | ✅ | ✅ | — | Composite-PK join-source CUD uses row-tuple IN on SQLite/Postgres/MySQL and a JOIN target form on SQL Server. Grouped/aggregated CUD remains rejected; ordered/`Distinct` shapes run against the plain filter, and paged/windowed shapes resolve through a keyed subquery over the full query SQL as a derived table (portable ORDER BY + paging; requires key columns). | `LiveProviderCompositePkBulkCudParityTests`, `ExecuteDeleteUpdateJoinSourceTests`, `LinqCompositePkExecuteCudTests`, `OrderedPagedBulkCudLiveTests` |
