...
| Raw SQL composition (`FromSqlRawAsync<T>`, `FromSqlInterpolatedAsync<T>`, `QueryUnchangedInterpolatedAsync<T>`) | ✅ | ✅ | ✅ | ✅ | ✅ | — | Constrained read-only SQL contract. Caller owns SQL shape and tenant predicates; nORM validates the query gate and binds values through provider parameters. | `LiveProviderRawSqlParityTests`, `RawSqlNameBasedMaterializationTests`, `TransactionIsolationTests` |
...
| Feature | SQLite | SqlServer | Postgres | MySQL | Runtime | Compiled | Caveats | Tests |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `string` methods (Contains, StartsWith, EndsWith, Substring, Trim*, Replace, IndexOf, Concat, Compare, CompareTo, ToUpper/Lower, char.ToUpperInvariant, `string.Format`, interpolated projections) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | `string.Format` support is the documented constant-template subset from `linq-support.md`. | `LiveProviderShapeParityTests`, `TypeConversionParityTests`, `LiveProviderCharStringPredicateParityTests`, `LiveProviderValueFunctionParityTests`, `LiveProviderStringFunctionParityTests` |
...
| `Guid` comparisons, equality, IN, static constants | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | — | `LinqGuidAndDistinctTests`, `TypeConversionParityTests`, `LiveProviderStaticValuePredicateParityTests` |
