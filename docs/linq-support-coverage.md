...
| Explicit-queryable correlated subqueries (`ctx.Query<X>()...` in predicates and projections) | `tests/CorrelatedScalarAggregateSubqueryTests.cs`, `tests/LinqSubqueryInProjectionTests.cs`, `tests/LinqParityFuzzTests.cs` (grouped/correlated pass), `tests/LiveProviderSubqueryFuzzParityTests.cs` |
