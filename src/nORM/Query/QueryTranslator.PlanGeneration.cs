    ...
    WHERE {timeParamName} >= {t1}.{_t._provider.Escape("__ValidFrom")} AND {timeParamName} < {t1}.{_t._provider.Escape("__ValidTo")}
                    ...
                    if (_t._isAggregate && _t._groupBy.Count == 0)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                var selectVisitor = new SelectClauseVisitor(_t._mapping, _t._groupBy, _t._provider, alias, ctx: _t._ctx) { CoerceDecimalProjectionsToReal = true | _t._coerceDecimalProjectionsToReal, ForceOrdinalStringProjections = _t._forceOrdinalStringProjections || (_t._provider.DefaultStringEqualityIsCaseInsensitive && _t._projection.Body.Type != typeof(string)), SharedParams = _t._params, SharedCompiledParams = _t._compiledParams, SharedParamConverters = _t._paramConverters };
                    ...
                    else
                    {
                        ...
                        {
                            var selectVisitor = new SelectClauseVisitor(_t._mapping, _t._groupBy, _t._provider, alias, ctx: _t._ctx) { CoerceDecimalProjectionsToReal = _t._isDistinct || _t._coerceDecimalProjectionsToReal, ForceOrdinalStringProjections = _t._forceOrdinalStringProjections || (_t._isDistinct && _t._provider.DefaultStringEqualityIsCaseInsensitive && _t._projection.Body.Type != typeof(string)), SharedParams = _t._params, SharedCompiledParams = _t._compiledParams, SharedParamConverters = _t._paramConverters };
