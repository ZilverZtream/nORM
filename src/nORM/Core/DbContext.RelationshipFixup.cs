    ...
    public partial class DbContext
    {
        ...
        private void FixupReferenceNavigations(EntityEntry entry, Queue<EntityEntry> queue)
        {
            ...
            foreach (var navProp in navProps)
            {
                ...
                // The FK may target a NON-PK (alternate) principal key — HasForeignKey(dep => dep.Fk,
                // prin => prin.AltKey). Resolve the principal key column THIS relationship actually references
                // (from the principal's Relation), so the reference-nav fixup persists the alternate-key value,
                // mirroring the composite and collection directions. Falls back to the PK for a convention
                // FK->PK relationship (no explicit alternate principal key), which is the common case.
                var pkColumn = principalMap.KeyColumns[0];
                foreach (var rel in principalMap.Relations.Values)
                {
                    if (!rel.IsComposite
                        && rel.DependentType == entry.Mapping.Type
                        && string.Equals(rel.ForeignKey.Name, fk.Name, StringComparison.Ordinal))
                    {
                        pkColumn = rel.PrincipalKey;
                        break;
                    }
                }
                // The principal's key is still default and will be generated by its INSERT when it is
                // either an explicitly DB-generated key ([DatabaseGenerated(Identity)]) OR a convention
                // store-generated key (a bare int PK). The latter has IsDbGenerated == false — the two are
                // mutually exclusive — so gating on IsDbGenerated alone left the convention-key reference
                // nav (the most common style) linking the dependent to the still-default key (0). Defer
                // both, so the FK is set after the principal's key is hydrated (ResolvePendingReferenceKeyFixups).
                var principalKeyPendingGeneration = pkColumn.IsDbGenerated
                    ? IsDefaultDbGeneratedKey(principal, principalMap)
                    : ReferenceEquals(pkColumn, principalMap.ConventionGeneratedKeyColumn)
                        && IsDefaultConventionKey(principal, pkColumn);
                if (principalKeyPendingGeneration && principalEntry.State == EntityState.Added)
                {
