...
namespace nORM.Core
{
    public partial class DbContext
    {
        ...
        /// <summary>
        /// True when the store-generated-key CONVENTION column holds its type's default value (0, or null
        /// for a nullable key) — meaning the database should generate it. A non-default value is honored on
        /// INSERT. Only ever called for a map that HAS a convention key (SQLite), so the column is non-null.
        /// </summary>
        private static bool IsDefaultConventionKey(object entity, Column conventionKey)
            => ChangeTracker.IsDefaultKeyValue(conventionKey.Getter(entity), conventionKey.Prop.PropertyType);
        ...
        {
            var convKey = map.ConventionGeneratedKeyColumn;
            if (convKey is null)
            {
                var affected = await ExecuteInsertBatchCore(cmd, map, batch, sql, paramIndex, ct,
                    _p.GetInsertColumns(map), HasDbGeneratedKey(map.KeyColumns)).ConfigureAwait(false);
                // Reader is closed now; hydrate any DB-computed/default non-key columns onto the entities.
                await MaybeReadBackStoreGeneratedColumnsAsync(cmd, map, batch, ct).ConfigureAwait(false);
                return affected;
            }

            // Store-generated-key convention (EF Core parity): a default-valued key is generated by the
            // database (the key column is omitted from the INSERT and the generated value read back); a
            // non-default value is inserted as-is (honored). The two kinds need different column lists and
            // different read-back, and mixing rows-with-read-back and rows-without in one multi-statement
            // command would misalign the result sets to entities. So execute the batch as contiguous runs of
            // the SAME kind — each a uniform command. Runs preserve the entities' existing order (which
            // self-referential ordering established), so an explicit-key parent still precedes a default-key
            // child across the run boundary.
            var total = 0;
            var i = 0;
            while (i < batch.Count)
            {
                var runDefault = IsDefaultConventionKey(batch[i].Entity ?? throw new InvalidOperationException("Entity is null"), convKey);
                var j = i + 1;
                while (j < batch.Count
                    && IsDefaultConventionKey(batch[j].Entity ?? throw new InvalidOperationException("Entity is null"), convKey) == runDefault)
                    j++;
                var run = batch.GetRange(i, j - i);
                sql.Clear();
                cmd.Parameters.Clear();
                var insertCols = runDefault ? map.InsertColumnsWithoutConventionKey : _p.GetInsertColumns(map);
                total += await ExecuteInsertBatchCore(cmd, map, run, sql, 0, ct, insertCols, runDefault).ConfigureAwait(false);
                i = j;
            }
            // All runs done and readers closed; hydrate any DB-computed/default non-key columns.
            await MaybeReadBackStoreGeneratedColumnsAsync(cmd, map, batch, ct).ConfigureAwait(false);
            return total;
        }

        /// <summary>
        /// Core batched-INSERT executor for one uniform run. <paramref name="insertCols"/> is the exact
        /// column set to write (the full insert columns, or — for a store-generated convention key at its
        /// default — those minus the key), and <paramref name="needsKeyReadBack"/> requests hydrating the
        /// generated key back onto each entity. The SQL builder and parameter binder are both driven by
        /// <paramref name="insertCols"/> so the positional parameters stay aligned.
        /// </summary>
        private async Task<int> ExecuteInsertBatchCore(DbCommand cmd, TableMapping map, List<EntityEntry> batch, StringBuilder sql, int paramIndex, CancellationToken ct, Column[] insertCols, bool needsKeyReadBack)
        {
            ...
            foreach (var entry in batch)
            {
                AppendInsertBatch(sql, map, paramIndex, insertCols, needsKeyReadBack);
                sql.Append(';');
        ...
        private async Task<int> ExecuteUpdateBatch(DbCommand cmd, TableMapping map, List<EntityEntry> batch, StringBuilder sql, int paramIndex, CancellationToken ct, bool deferAccept = false)
        {
            ...
            // Reader closed; hydrate any DB-computed non-key columns that the UPDATE recomputed (e.g. a
            // GENERATED ALWAYS AS column) so the tracked entity matches the row.
            await MaybeReadBackStoreGeneratedColumnsAsync(cmd, map, emitted, ct).ConfigureAwait(false);
        ...
        /// <summary>
        /// After a successful INSERT/UPDATE, reads database-generated non-key column values (computed columns
        /// and store-generated defaults) back onto the tracked entities so the in-memory objects match the row,
        /// matching EF Core. No-op unless the mapping has such columns, so ordinary writes are unaffected. Runs
        /// one keyed SELECT per parameter-budget chunk over the same connection and transaction as the batch, so
        ...
        private async Task ReadBackStoreGeneratedColumnsCoreAsync(DbTransaction? tx, TableMapping map, IReadOnlyList<object> entities, CancellationToken ct)
        {
            ...
            var tenantExtra = (Options.TenantProvider != null && map.TenantColumn != null) ? 1 : 0;
            var chunk = _p.MaxParameters == int.MaxValue
                ? entities.Count
                : Math.Max(1, (_p.MaxParameters - tenantExtra) / perEntity);

            string KeyOf(object[] parts)
            {
                var sb = new StringBuilder();
                foreach (var p in parts)
                {
                    sb.Append('|');
                    if (p is null) sb.Append("\0n");
                    else if (p is byte[] b) sb.Append("b:").Append(Convert.ToBase64String(b));
                    else sb.Append("v:").Append(Convert.ToString(p, System.Globalization.CultureInfo.InvariantCulture));
                }
                return sb.ToString();
            }

            object ModelKeyPart(Column kc, object? raw)
            {
                if (raw is null || raw is DBNull) return null!;
                if (kc.Converter != null) return kc.Converter.ConvertFromProvider(raw)!;
                var target = Nullable.GetUnderlyingType(kc.Prop.PropertyType) ?? kc.Prop.PropertyType;
                return raw.GetType() == target ? raw : (nORM.Query.MaterializerFactory.ConvertDbValue(raw, target) ?? raw);
            }

            for (int offset = 0; offset < entities.Count; offset += chunk)
            {
                var count = Math.Min(chunk, entities.Count - offset);
                var byKey = new Dictionary<string, object>(count);
                for (int i = 0; i < count; i++)
                {
                    var entity = entities[offset + i];
                    var parts = new object[map.KeyColumns.Length];
                    for (int k = 0; k < map.KeyColumns.Length; k++)
                        parts[k] = map.KeyColumns[k].Getter(entity)!;
                    byKey[KeyOf(parts)] = entity;
                }
                ...
                var sb = new StringBuilder("SELECT ").Append(selectList).Append(" FROM ").Append(map.EscTable).Append(" WHERE ");
                var conditions = new List<string>(count);
                int pi = 0;
                for (int i = 0; i < count; i++)
                {
                    var entity = entities[offset + i];
                    var pkConds = new List<string>(map.KeyColumns.Length);
                    foreach (var kc in map.KeyColumns)
                    {
                        pkConds.Add($"{kc.EscCol}={_p.ParamPrefix}g{pi}");
                        var rawKey = kc.Getter(entity);
                        cmd.AddParam($"{_p.ParamPrefix}g{pi++}", kc.Converter != null ? kc.Converter.ConvertToProvider(rawKey) : rawKey);
                    }
                    conditions.Add($"({string.Join(" AND ", pkConds)})");
                }
                if (Options.TenantProvider != null && map.TenantColumn != null)
                {
                    var tenantCol = RequireTenantColumn(map, "store-generated read-back");
                    sb.Append('(').Append(string.Join(" OR ", conditions)).Append($") AND {tenantCol.EscCol} = {_p.ParamPrefix}gtenant");
                    cmd.AddParam($"{_p.ParamPrefix}gtenant", GetRequiredTenantId(map, "store-generated read-back"));
                }
                else
                {
                    sb.Append(string.Join(" OR ", conditions));
                ...
                await using var reader = await cmd.ExecuteReaderWithInterceptionAsync(this, CommandBehavior.Default, ct).ConfigureAwait(false);
                var keyCount = map.KeyColumns.Length;
                while (await reader.ReadAsync(ct).ConfigureAwait(false))
                {
                    var keyParts = new object[keyCount];
                    for (int k = 0; k < keyCount; k++)
                        keyParts[k] = ModelKeyPart(map.KeyColumns[k], reader.GetValue(k));
                    if (!byKey.TryGetValue(KeyOf(keyParts), out var entity))
                        continue;
                    for (int j = 0; j < genCols.Length; j++)
                    {
                        var raw = reader.GetValue(keyCount + j);
                        if (raw is DBNull)
                            continue; // leave the CLR value; a generated column returning NULL is not a value to hydrate
                        var col = genCols[j];
                        var target = Nullable.GetUnderlyingType(col.Prop.PropertyType) ?? col.Prop.PropertyType;
                        var modelVal = col.Converter != null
                            ? col.Converter.ConvertFromProvider(raw)
                            : (raw.GetType() == target ? raw : nORM.Query.MaterializerFactory.ConvertDbValue(raw, target));
                        col.Setter(entity, modelVal);
                    }
                }
            }
        }
