...
/// <summary>
/// TX-1: Verifies that SaveChanges defers AcceptChanges / ChangeTracker.Remove when the
...
///
/// TX-2: Verifies that the retry loop is skipped entirely when an explicit or ambient
...
{
    // TX-1 entity: AUTOINCREMENT key, used for update/delete tests.
    [Table("TxStateItem")]
    ...
    // TX-2 entity: manual (non-generated) key, so nORM includes it in INSERT
    ...
    // ─── TX-1: AcceptChanges deferred for external transaction ───────────────

    /// <summary>
    /// TX-1 regression: Under an external transaction that the caller later rolls back,
    ...
    public async Task SaveChanges_ExternalTx_RolledBack_ModifiedEntitiesRemainModified()
    {
        ...
        await using (cn) await using (ctx)
        {
            ...
            // TX-1 fix: AcceptChanges must NOT run under a non-owned transaction.
    ...
    /// <summary>
    /// TX-1: Self-owned transaction (the normal happy path) must still call AcceptChanges.
    /// Entities must become Unchanged after a successful SaveChanges with no external tx.
    /// This guards against the TX-1 fix over-deferring and breaking the common path.
    /// </summary>
    [Fact]
    public async Task SaveChanges_OwnedTx_EntitiesAcceptedUnchanged_AfterCommit()
    {
        ...
        await using (cn) await using (ctx)
        {
            ...
            // TX-1: Owned-tx path must still call AcceptChanges.
    ...
    /// <summary>
    /// TX-1 regression: Under a rolled-back external transaction, Deleted entities must
    ...
    public async Task SaveChanges_ExternalTx_DeleteRolledBack_EntityRemainsTrackedAsDeleted()
    {
        ...
        await using (cn) await using (ctx)
        {
            ...
            // TX-1 fix: ChangeTracker.Remove must NOT run under a non-owned transaction.
    ...
    /// <summary>
    /// TX-1: After an external transaction is committed by the caller, SaveChanges on a
    ...
    public async Task SaveChanges_ExternalTx_AfterCallerCommits_SubsequentSaveSucceeds()
    {
        ...
        await using (cn) await using (ctx)
        {
            ...
            // Caller commits — entity is still in Added state (TX-1 fix: AcceptChanges deferred)
    ...
    // ─── TX-2: Retry loop bypassed under external transaction ────────────────

    /// <summary>
    /// TX-2 regression: When an explicit (DbTransaction) external transaction is active,
    ...
    public async Task SaveChanges_ExplicitExternalTx_RetryableFailure_DoesNotRetry()
    {
        ...
        await using (cn) await using (ctx)
        {
            ...
            // TX-2 fix: ShouldRetry was never consulted because external tx skips the retry loop.
            Assert.Equal(0, retryCount);
        }
    }

    /// <summary>
    /// TX-2: With NO external transaction and a retry policy, a retryable failure DOES
    /// consult the retry policy. This verifies the TX-2 guard does not break normal retries.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// TX-2: An ambient System.Transactions.TransactionScope also counts as an external
    ...
    public async Task SaveChanges_AmbientTransactionScope_RetryableFailure_DoesNotRetry()
    {
        ...
        var opts = new DbContextOptions
        {
            ...
            // we only need Transaction.Current != null for the TX-2 guard to fire.
        ...
        await using (cn) await using (ctx)
        {
            ...
            // TX-2 fix: ambient scope causes Transaction.Current != null → retry loop skipped.
            Assert.Equal(0, retryCount);
