...
public partial class ScaffoldingContractDocTests
{
    ...
    public void Doc_and_cli_pin_repeatable_table_filter_for_literal_commas()
    {
        ...
        Assert.Contains("`noPluralize`, `useDatabaseNames`, and `force`", doc, StringComparison.Ordinal);
        Assert.Contains("comma-separated or array table/schema defaults", doc, StringComparison.Ordinal);
        Assert.Contains("defaults are ignored so they cannot expand the explicit selection", doc, StringComparison.Ordinal);
        Assert.Contains("options take precedence over configuration file values", doc, StringComparison.Ordinal);
        ...
        Assert.Contains("`noPluralize`, `useDatabaseNames`, and `force`", cliReadme, StringComparison.Ordinal);
        Assert.Contains("defaults are ignored so they cannot expand the explicit selection", cliReadme, StringComparison.Ordinal);
        ...
        Assert.Contains("`noPluralize`, `useDatabaseNames`, and `force`", rootReadme, StringComparison.Ordinal);
        Assert.Contains("defaults are ignored so they cannot expand the explicit selection", rootReadme, StringComparison.Ordinal);
