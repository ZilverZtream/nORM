...
//<summary>
//Verifies that <see cref="TransactionManager.RollbackAsync"/> always uses
...
public class TransactionRollbackTests
{
 ...
 // Use reflection to get the Result property
    ...
    {
 // Simulate: operation fails, caller token is canceled before rollback.
 ...
 // because fix makes it always use CancellationToken.None internally.
    ...
    {
 // Even if the token is already canceled when RollbackAsync is called,
