...
//<summary>
//Verifies that subquery Contains on nullable columns uses null-safe SQL semantics.
...
public class ContainsNullSemanticsTests
{
 ...
 // ─── null value in source and target ────────────────────────────

    [Fact]
    public async Task Contains_NullInSourceSubquery_NullTargetRow_ReturnsNullRow()
    {
 // When source subquery contains null AND target row has null,
 ...
 // The null target row (id=4) should be returned because null is in the source.
    ...
    {
 // non-regression: non-null rows not in the source are still excluded.
        var (cn, ctx) = CreateContext();
        using var _cn = cn;
        using var _ctx = ctx;
 ...
 // ─── compile-time null Contains ────────────────────────────────

    [Fact]
    public async Task Contains_CompileTimeNull_SourceHasNull_ReturnsAllTargetRows()
    {
 // Contains((int?)null) with a source that has null → the WHERE clause
 ...
 // All rows should be returned (condition is universally true)
    ...
    {
 // Contains((int?)null) when source has no nulls → WHERE clause is always false.
        var (cn, ctx) = CreateContext();
        using var _cn = cn;
        using var _ctx = ctx;
 ...
 // ─── null in source but target has no nulls ─────────────────────

    [Fact]
    public async Task Contains_NullInSource_TargetHasNoNulls_NullRowsNotInTarget()
    {
 // non-regression: if the source has null but the target has no null rows,
