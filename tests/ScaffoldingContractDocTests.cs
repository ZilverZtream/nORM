...
public class ScaffoldingContractDocTests
{
    ...
    public void Doc_catalogs_all_scaffold_diagnostic_codes()
    {
        ...
        Assert.Contains("SQLite declared `UUID`, `JSON`, and `XML`, SQL Server `xml`, PostgreSQL `json`/`jsonb`/`xml`/`uuid` plus safe scalar arrays/simple enums, and MySQL `json`/`year`/simple `enum(...)` are scaffolded as supported storage", doc, StringComparison.Ordinal);
