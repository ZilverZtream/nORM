...
public class ScaffoldingContractDocTests
{
    ...
    public void Doc_catalogs_all_scaffold_diagnostic_codes()
    {
        ...
        Assert.Contains("SQLite declared `UUID`, `JSON`, and `XML`, SQL Server `xml`, PostgreSQL `json`/`jsonb`/`xml`/`uuid`, and MySQL `json` are scaffolded as supported scalar storage", doc, StringComparison.Ordinal);
