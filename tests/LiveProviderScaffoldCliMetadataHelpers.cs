...
public sealed partial class LiveProviderScaffoldCliParityTests
{
        ...
        string statusDefaultName)
    {
        ...
        var statusDefaultConstraint = provider.Escape(statusDefaultName);

        var createSql = kind switch
        {
            ProviderKind.SqlServer =>
                $"CREATE TABLE {table} ({id} int NOT NULL PRIMARY KEY, {name} nvarchar(80) COLLATE Latin1_General_BIN2 NOT NULL CONSTRAINT {defaultConstraint} DEFAULT ('new'), {status} nvarchar(32) NOT NULL CONSTRAINT {statusDefaultConstraint} DEFAULT (LOWER(N'NEW')), {payload} varbinary(4) NOT NULL DEFAULT 0xDEADBEEF, {nameLength} AS (LEN({name})) PERSISTED, CONSTRAINT {check} CHECK (LEN({name}) > 0))",
            ProviderKind.Postgres =>
                $"CREATE TABLE {table} ({id} int NOT NULL PRIMARY KEY, {name} varchar(80) COLLATE \"C\" NOT NULL DEFAULT 'new', {status} varchar(32) NOT NULL DEFAULT lower('NEW'), {payload} bytea NOT NULL DEFAULT '\\xDEADBEEF'::bytea, {createdAt} timestamp without time zone NOT NULL DEFAULT (now() AT TIME ZONE 'utc'), {nameLength} integer GENERATED ALWAYS AS (char_length({name})) STORED, CONSTRAINT {check} CHECK (char_length({name}) > 0))",
            ProviderKind.MySql =>
                $"CREATE TABLE {table} ({id} int NOT NULL PRIMARY KEY, {name} varchar(80) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL DEFAULT 'new', {status} varchar(32) NOT NULL DEFAULT (LOWER('NEW')), {payload} varbinary(4) NOT NULL DEFAULT 0xDEADBEEF, {nameLength} int GENERATED ALWAYS AS (CHAR_LENGTH({name})) STORED, CONSTRAINT {check} CHECK (CHAR_LENGTH({name}) > 0))",
            ProviderKind.Sqlite =>
                $"CREATE TABLE {table} ({id} INTEGER NOT NULL PRIMARY KEY, {name} TEXT COLLATE NOCASE NOT NULL DEFAULT 'new', {status} TEXT NOT NULL DEFAULT (lower('NEW')), {payload} BLOB NOT NULL DEFAULT X'DEADBEEF', {nameLength} INTEGER GENERATED ALWAYS AS (length({name})) VIRTUAL, CONSTRAINT {check} CHECK (length({name}) > 0))",
