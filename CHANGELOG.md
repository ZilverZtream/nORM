...
- Store-generated primary keys by convention (EF Core parity): a single-column
  integer key with no explicit value-generation config is now store-generated when
  its value is default (`0`) and honored when a non-default value is supplied, on all
  four providers (SQLite rowid alias, SQL Server `IDENTITY` with `IDENTITY_INSERT`
  for explicit values, PostgreSQL `GENERATED BY DEFAULT AS IDENTITY`, MySQL
  `AUTO_INCREMENT`). Opt out per entity with `[DatabaseGenerated(None)]` or globally
  with `DbContextOptions.UseStoreGeneratedKeyConvention = false` (the escape hatch for
  an existing table whose key column is not an identity/auto-increment column).
